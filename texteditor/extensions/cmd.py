...
class cmd(textwidget.TextWidget):
    ...
    def __init__(self, master, **kw):
        ...
        self.addMenucheckbtn(label=_("Read only"), fn=self.readonlymode, variable=self.readonlybtn)
    ...
    def checkcmds(self, *args):
        ...
        if command == 'exit':
            ...
            append('end', '\n')
            append('end', _("This will open console defined by defconsole value in configuration file. Please don't run any shell!"))
            cmd = get_config.getvalue('cmd', 'defconsole')

            if cmd == ("bash" or "xonsh" or "zsh" or "sh"):
                messagebox.showerror(title="Error", message="This application cannot run shell, use terminal emulator instead.\nProgram: "+cmd)
