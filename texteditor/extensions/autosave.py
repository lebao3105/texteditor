...
class AutoSave:
    ...
    * start : Start the timing loop
    ...
    useTime: float = float(
        get_config.GetConfig.getvalue("filemgr", "autosave-time"))
    ...
    def openpopup(self):
        ...
        label = Label(
            askwin,
            text=self._("Select autosave time (minutes)\nAutosave function will be launched after a time."),
        )
        label2 = Label(
            askwin,
            text=self._("Please note that Autosave only saves the current selected tab."),
        )
        ...
        cancelbtn = Button(askwin, text=self._("Cancel"),
                           command=lambda: askwin.destroy())
