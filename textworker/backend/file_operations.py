...
searchdir = config.getkey(
    "editor", "searchdir", noraiseexp=True, restore=True, getbool=False
)
autosave = config.getkey(
    "editor", "autosave", noraiseexp=True, restore=True
) or config.getkey("extensions.autosave", "enable", noraiseexp=True)
...
class FileOperations:
    ...
    def DynamicEditorChanges(self):
        ...
        self.thread = threading.Thread(target=subfn(), daemon=True)
        self.thread.run()
...
class MultiEditorsSupport(FileOperations):
    ...
    def __init__(
        self, Tabber: typing.Any | wx.Notebook, configs: dict[str, typing.Any]
    ):
        self.tabs = Tabber
