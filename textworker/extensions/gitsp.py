...
class GitSupport:
    ...
    def InitFolder(self, path: str):
        ...
        if not os.path.isdir(path):
            raise Exception("textworker.extensions.gitsp[INIT]: Folder not found: " + path)
        
        if not os.path.isdir(CraftItems(path , ".git")):
            logger.error("textworker.extensions.gitsp: Not a git repo (or any of the parent directories) - .git not found anywhere")
            return None
        ...
        # Repo branch & remote
        self.Branch["current"] = str(subprocess.check_output([self.GitPath, "branch"])).removeprefix("* ")
        self.Branch["list"] = str(subprocess.check_output([self.GitPath, "branch", "-r"]))
...
class GitSupportGUI(GitSupport):
    ...
    def __init__(self, Parent):
        ...
        self.Blank.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial" ) )
        ...
        self.RepoPath.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
        ...
        self.Header2.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )

        bSizer1.Add( self.Header2, 0, wx.ALL, 5 )

        self.NewList = wx.ListCtrl(self.Panel, style=wx.LC_AUTOARRANGE|wx.LC_ICON )
